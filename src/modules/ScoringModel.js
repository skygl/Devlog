...
async function predictScore(testDomInfo) {
    if (!model) {
        let doms = await DomService.findDom({scored: true, endDate: getDate(new Date(), {day: -2})});
        await loadOrTrainModel(doms);
        model = tempModel;
