...
async function saveModel(model) {
    return model.save('file://' + __dirname + '/model');
}

export async function testModel(testDomInfo) {
    if (!model) {
        if (!fs.existsSync(path.resolve(__dirname, 'model/model.json'))) {
            const [xTrain, yTrain, xTest, yTest] = await getDomData(.2);
...
const getDomData = (async (testSplit) => {
    return tf.tidy(() => {
        const dataByScore = [];
